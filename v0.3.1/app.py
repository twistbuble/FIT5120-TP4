...
def add_child():
    ...
    try:
        child = Child(child_gender=gender, child_name=name, child_age=age, child_intolerance = intolerance, user_id=1234)
...
def rate_recipe():
    ...
    try:
        recipe = Meal_Recipes.query.filter_by(recipe_index=recipe_id).first()
        if recipe:
            if User_Ratings.query.filter_by(user_id=1234, recipe_index=recipe_id).update({User_Ratings.rating: ratings}) == 0:
                this_rating = User_Ratings(user_id=1234, recipe_index=recipe_id, rating=ratings)
...
def meal_recipes():
    ...
    try:
        ...
        recommend_recipes = User_Recommendations.query.filter_by(user_id=1234).first()
...
def get_pdf_report():
    ...
    try:
        ...
        if child:
            ...
            if child[0].child_age == 0:
                w.gender = 'Male'
            elif child[0].child_age == 1:
                w.gender = 'Female'
            else:
                w.gender = 'Other'
            w.write_info('info')
